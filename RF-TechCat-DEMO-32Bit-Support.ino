...
struct CodeItem {
  long code;
  int bitLength;   // <--- NOVÉ POLIE: Dĺžka signálu v bitoch (1-32)
...
int lastValidBitLength = 0;  // <--- NOVÁ PREMENNÁ: Uložíme počet bitov zachyteného signálu
...
<body>
  ...
  function showPopup(title, isSuccess, data = null) {
    ...
    if (data && data.code !== undefined) {
      ...
      bitLengthDiv.textContent = data.bitLength || '24'; // <--- Zobrazujeme dĺžku bitov
      if (data.status) {
  ...
  function validateCode(codeStr) {
    ...
    if (code > 4294967295) { // <--- ZMENENÉ NA 32-BITOVÝ LIMÍT
  ...
  function validateAndSaveManualCode() {
    ...
    const signalData = {
      ...
      bitLength: 24 // Default, keďže manuálne zadávanie nevie dĺžku
    };
      ...
      .then(codes => {
        ...
        if (currentCodeCount > previousCodeCount) {
          ...
          const signalData = {
            ...
            bitLength: lastCode.bitLength // <--- Pridané!
          };
  ...
  function toggleRollingCodes() {
    ...
    if (isRolling) {
      ...
      if (fromCode < 1 || fromCode > 4294967295 || toCode < 1 || toCode > 4294967295) { // <--- ZMENENÉ
        ...
        codes.forEach(item => {
          ...
          div.innerHTML = `
            <div class="code-info">
              ...
              <div style="font-family:monospace;color:#666">Bitov: ${item.bitLength}</div> <!-- <--- ZOBRAZUJEME BITOVÚ DĹŽKU -->
...
void loadCodesFromEEPROM() {
  ...
  for (int i = 0; i < MAX_CODES; i++) {
    ...
    // Ak je bitLength mimo platného rozsahu (1-32), nastavme ho na 24 ako default
...
void saveCodeToEEPROM(long code, int bitLength, const char* name) {
  ...
  CodeItem item;
  item.code = code;
  item.bitLength = bitLength; // <--- ULOŽÍME SKUTOČNÚ DĹŽKU
...
void deleteCodeFromEEPROM(long code) {
  ...
  for (int i = 0; i < MAX_CODES; i++) {
    if (i < codeCount) {
      ...
      CodeItem empty = {0, 0, ""}; // <--- VYPLNIME AJ bitLength
...
";
      ...
      xTaskCreatePinnedToCore(
          [](void *param) {
              ...
              for (long code = fromCode; code <= toCode; code++) {
                  ...
                  mySwitch.send(code, 24); // Všetky rolling kódy odosielame ako 24-bitové (pre kompatibilitu)
                  count++;
  ...
  // === TRANSMIT HANDLER (POUŽÍVA ULOŽENÚ DĹŽKU BITOV) ===
  server.on("/transmit", HTTP_POST, [](AsyncWebServerRequest *request){
    if (request->hasParam("code", true)) {
      ...
      if (code > 0 && code <= 4294967295) { // <--- ZMENENÉ NA 32-BITOVÝ LIMÍT
        int bitLength = getBitLengthForCode(code);
        mySwitch.send(code, bitLength); // <--- ODOSIELAME S PRESNOU DĹŽKOU
  ...
  server.on("/transmit3", HTTP_POST, [](AsyncWebServerRequest *request){
    if (request->hasParam("code", true)) {
      long code = request->getParam("code", true)->value().toInt();
      if (code > 0 && code <= 4294967295) { // <--- ZMENENÉ
        int bitLength = getBitLengthForCode(code);
        for (int i = 0; i < 3; i++) {
          mySwitch.send(code, bitLength); // <--- ODOSIELAME S PRESNOU DĹŽKOU
          delay(1000);
  ...
  server.on("/saveManual", HTTP_POST, [](AsyncWebServerRequest *request){
    if (request->hasParam("code", true) && request->hasParam("name", true)) {
      ...
      if (code > 0 && code <= 4294967295) {
        saveCodeToEEPROM(code, 24, name.c_str()); // Manuálne zadávanie = 24-bitový predpoklad
...
void loop() {
  if (isReceiving && (millis() - receiveStartTime) < 3000) {
    if (mySwitch.available()) {
      ...
      int bits = mySwitch.getReceivedBitlength(); // <--- ZÍSKAME SKUTOČNÝ POČET BITOV
      
      // Prijmeme akýkoľvek platný signál (1–32 bity)
      if (value > 0 && bits >= 1 && bits <= 32) {
        lastValidCode = value;
        lastValidBitLength = bits; // <--- ULOŽÍME DĹŽKU
        signalReceived = true;
...
// === NOVÁ FUNKCIA: ZÍSKA DĹŽKU BITOV Z ULOŽENÉHO KÓDU ===
int getBitLengthForCode(long code) {
  ...
  return 24; // Predpokladaná hodnota pre kompatibilitu (staré kódy)
}
