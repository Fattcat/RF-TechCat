...
// === Štruktúra pre kódy (kompatibilná s RCSwitch formátom) ===
struct CodeItem {
    long code;
    int bitLength;   // Dĺžka signálu v bitoch (1-64)
...
// === HTML stránka (skrátená verzia - použite tú istú ako v RCSwitch kóde) ===
...
.signal-animation {height: 100px;margin: 15px 0;background: #000;border-radius: 10px;overflow: hidden;position: relative;border: 1px solid #0f0;box-shadow: 0 0 10px rgba(0, 255, 0, 0.3);}
#spectrumCanvas {width: 100%;height: 100%;display: block;}
.frequency-scale {display: flex;justify-content: space-between;margin-top: 5px;font-family: monospace;font-size: 12px;color: #0f0;padding: 0 10px;}
...
function addToSendLog(code, status = 'odoslaný') {
  ...
  const entry = { code: code, timestamp: timestamp, status: status };
...
void loadCodesFromEEPROM() {
  ...
  for (int i = 0; i < MAX_CODES; i++) {
    ...
    // Validácia bitLength
    if (item.bitLength < 1 || item.bitLength > 64) {
      item.bitLength = 24;
    }
...
// ✅ KĽÚČOVÁ FUNKCIA: Konverzia long kódu na bajty pre CC1101
void longToBytes(long value, uint8_t* bytes, int length) {
  for (int i = 0; i < length; i++) {
    bytes[i] = (value >> (i * 8)) & 0xFF;
  }
}

// ✅ KĽÚČOVÁ FUNKCIA: Odoslanie kódu cez CC1101
void transmitCodeCC1101(long code, int bitLength) {
  // CC1101 pracuje s bajtmi, nie s long
  int byteLength = (bitLength + 7) / 8;
  uint8_t packet[8] = {0};
  
  // Konverzia long na bajty (little-endian)
  for (int i = 0; i < byteLength && i < 8; i++) {
    packet[i] = (code >> (i * 8)) & 0xFF;
  }
  
  // Odoslanie cez CC1101
  radio.setModeIdle();
  radio.transmit(packet, byteLength);
  radio.setModeRX();
  
  Serial.printf("📤 CC1101 Odoslaný kód: %ld (%d bitov)\n", code, bitLength);
}
...
void setup() {
  ...
  radio.setPower(10);          // Výkon v dBm (0-12)
  radio.setModeRX();           // Spustiť prijímanie
  
  Serial.println("✅ CC1101 inicializovaný na 433.92 MHz");
  Serial.println("   Modulácia: OOK/ASK");
  Serial.println("   Výkon: 10 dBm");
  Serial.println("   Citlivosť: -104 dBm");
    ...
    if (xTaskCreatePinnedToCore(
      [](void *param) {
        ...
        for (long code = fromCode; code <= toCode; code++) {
          ...
          // Dynamická dĺžka bitov
          int bitLength = (code <= 16777215) ? 24 : 32;
          transmitCodeCC1101(code, bitLength);
          count++;
...
void loop() {
  if (isReceiving && (millis() - receiveStartTime) < 3000) {
    ...
    if (len > 0) {
      // Konverzia bajtov na long
      long value = 0;
      for (int i = 0; i < len && i < 4; i++) {
        value |= ((long)packet[i] << (i * 8));
      }
      
      // Detekcia dĺžky bitov (jednoduchá heuristika)
      int bits = len * 8;
      if (value > 0 && bits >= 8 && bits <= 64) {
        lastValidCode = value;
        lastValidBitLength = bits;
        Serial.printf("📡 CC1101 Zachytený signál: %ld (%d bitov)\n", value, bits);
      }
    }
  } else if (isReceiving) {
    isReceiving = false;
    if (lastValidCode != -1) {
      ...
      Serial.println("❌ CC1101 Žiadny signál");
