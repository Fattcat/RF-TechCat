...
// === Funkcia na výpis detailov signálu ===
void printSignalDetails(uint8_t* buffer, uint8_t len, int rssi) {
  ...
  if (len >= 3) {
    ...
    Serial.println("❌ Nedostatok dát na interpretáciu ako 24-bitový kód.");
  }
...
void setup() {
  ...
  // === NOVÝ HANDLER PRE MANUÁLNE ULOŽENIE KÓDU ===
  server.on("/saveManual", HTTP_POST, [](AsyncWebServerRequest *request){
    if (request->hasParam("code", true) && request->hasParam("name", true)) {
      ...
      if (code > 0 && code <= 16777215) {
        ...
        request->send(200, "text/plain", "Kód úspešne uložený");
...
void loop() {
  if (isReceiving && (millis() - receiveStartTime) < 3000) {
    ...
    if (lastValidCode != -1) {
      ...
      Serial.println("❌ Žiadny platný signál nebol zachytený.");
    }
  }
  delay(10);
}
